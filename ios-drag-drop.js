(function(doc) {
    ...
    function _exposeIosHtml5DragDropShim(config) {
        ...
        function main() {
            ...
            } else if(config.offsetToDrag) {
                doc.addEventListener("touchstart", touchstartOffset(config.offsetToDrag), {passive: false});
            } else {
        ...
        DragDrop.prototype = {
            ...
            dragend: function(event) {
                ...
                // start new code
                // missing dataTransfer in drag end event
                dragendEvt.dataTransfer = {
        ...
        function touchstartOffset(offset) {
            return function(evt) {
                var draggable = false;
                var original = evt.target;
                var el = original;
                if( evt.targetTouches && !evt._ignore ) {
                    var clientX = evt.targetTouches[0].clientX;
                    var clientY = evt.targetTouches[0].clientY;
                    do {
                        if (elementIsDraggable(el)) {
                            ...
                            var onReleasedItem = function() {
                                ...
                                if( draggable ) {
                                    ...
                                    original.click();
                                    original.focus();
                                    // fire original event, but we can ignore it now
                                  /* this doesn't seem to work
                                   var newEvent;
                                   try {
                                   newEvent = document.createEvent('TouchEvent');
                                   newEvent.initTouchEvent('touchstart', true, true);
                                   } catch( err ) {
                                   try {
                                   newEvent = document.createEvent('UIEvent');
                                   newEvent.initUIEvent('touchstart', true, true);
                                   } catch( err ) {
                                   newEvent = document.createEvent('Event');
                                   newEvent.initEvent('touchstart', true, true);
                                   }
                                   }
                                   newEvent.targetTouches = evt.targetTouches;
                                   newEvent.touches = evt.touches;
                                   newEvent.changedTouches = evt.changedTouches;

                                   newEvent._ignore = true;
                                   original.dispatchEvent(newEvent);
                                   */
                                }
                            };

                            var measureItem = function(moveEvt) {

                                var dx = moveEvt.targetTouches[0].clientX - clientX;
                                var dy = moveEvt.targetTouches[0].clientY - clientY;
                            ...
                            var move = onEvt(el, 'touchmove', measureItem, this);
